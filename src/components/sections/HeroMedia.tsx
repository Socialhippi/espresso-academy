 ...
 * loads. The 400ms shutter (`hero-media-in`) runs on the frame and never on the H1: the headline
 * is the LCP element at 390px and an entry animation on it would defer the paint Lighthouse times.
 *
 ...
 */
export function HeroMedia({ image, alt = "", slot, className }: HeroMediaProps) {
  const frame = cn(
  ...
  if (publicFileExists(VIDEO_PATH) && publicFileExists(POSTER_PATH)) {
    return (
      <div className="hero-media-in">
        ...
        {/* The same frame as a still: every phone, and anyone who has asked for less motion. */}
        <Image
          src={POSTER_PATH}
          alt={alt}
          width={1200}
